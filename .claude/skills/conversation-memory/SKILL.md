...
description: >
  ...
  `reviews/` for multi-agent memory sharing and conversation continuity.
  ...
  (Tracks A–E). Also trigger when the user says "save to reviews" or asks to
...
a track (A–E), and save it to `reviews/` with a deterministic filename. The
...
2. **Determine version** — List existing files for the same date and track:
   ```bash
   ls reviews/$(date +%Y%m%d)_TRACK_{X}_version_*.md 2>/dev/null | sort -V | tail -1
   ```
...
5. **Save** — Write to `reviews/{filename}`.
...
ls reviews/*_TRACK_B_*.md

# Latest version for a track on a given date
ls reviews/20260208_TRACK_C_*.md | sort -V | tail -1

# All tracks discussed on a date
ls reviews/20260208_*.md | sed 's/.*TRACK_\([^_]*\).*/\1/' | sort -u

# Full-text search across all summaries
grep -rl "multifractal" reviews/

# Count summaries per track
for t in A B C D D1 D2 D3 E; do
  echo "Track $t: $(ls reviews/*_TRACK_${t}_*.md 2>/dev/null | wc -l)"
done
```

## Example

**File**: `reviews/20260208_TRACK_D1_version_1.md`
